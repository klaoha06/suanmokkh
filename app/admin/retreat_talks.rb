ActiveAdmin.register RetreatTalk do
	batch_action :unpublish do |ids|
	    RetreatTalk.find(ids).each do |retreat_talk|
	      retreat_talk.draft = true
	      retreat_talk.save
	    end
	    redirect_to collection_path, alert: "Audios have been unpublished."
  end
	batch_action :publish do |ids|
	    RetreatTalk.find(ids).each do |retreat_talk|
	      retreat_talk.draft = false
	      retreat_talk.save
	    end
	    redirect_to collection_path, alert: "Audios have been published."
  end
	menu priority: 3
	config.per_page = 12
